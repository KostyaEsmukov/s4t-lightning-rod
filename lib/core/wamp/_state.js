...
function _wampStateConnectionOnOpen() {
    ...
    if (boardStatus === Board.getState().REGISTRATION_STATUSES.NEW) {
        ...
        provisioningChain = provisioningChain
            .then(_provisioningRegisterBoard.bind(this));
    ...
    provisioningChain = provisioningChain
        .then(_provisioningBoardConnection.bind(this));

    provisioningChain
        .then(function () {
            return Drivers.checkDrivers();
        });

    provisioningChain.done();
}

function _provisioningRegisterBoard() {
    var boardCode = Board.getState().getBoardCode();

    return this._wampConnection.getSession()
        .call("s4t.board.provisioning", [boardCode])
...
function _provisioningBoardConnection() {
    var boardCode = Board.getState().getBoardCode();
    ...
    this._wampConnection.getSession()
        .publish(connectionTopic, [boardCode, 'connection',
                this._wampConnection.getSession().getId()]
        );
}
