                        ...
                        Q.delay(2000 * mountingDriversPromises.length)  // todo delay is hardcoded
                            .then(function () {
                                ...
                                return exports.unmountDriver([driverName, true]) // todo
                                    .then(function () {
                                        return session_drivers.call('s4t.board.driver.updateStatus', [Board.getState().getBoardCode(), driverName, "unmounted"]);  // todo
                                    })
                                    .then(function () {
                            ...
                            .then(function () {
                                return session_drivers.call('s4t.board.driver.updateStatus', [Board.getState().getBoardCode(), driverName, "mounted"]); // todo
                            })
                    );
...
Drivers.mountDriver = function (driverName, remote, mirrorBoard) {
    ...
    return fspromised.stat(mountpoint)
        ...
        .then(function () {
            return AttachMountpoint(driverName, mountpoint, remote, mirrorBoard, d); // todo
        });
};
