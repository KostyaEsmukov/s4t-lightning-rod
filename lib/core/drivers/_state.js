...
var DriverModule = require('./driver-module');
var GlobalDriversConfigItem = require('./global-drivers-config-item');
...
function readGlobalDriversConfig() {
...
/**
 * Reads drivers global config file
 *
 * @returns {Q.Promise} GlobalDriversConfigItem[]
 */
DriversState.prototype.readGlobalDriversConfig = function () {
    return readGlobalDriversConfig()
        .then(function (conf) {
            var driversConf = conf.drivers;
            var driversKeys = helpers.objectKeys(driversConf);
            var res = [];
            for (var i = 0; i < driversKeys.length; i++) {
                var driverName = driversKeys[i];
                res.push(new GlobalDriversConfigItem(driversConf[driverName], driverName));
            }
...
/**
 * Reads driverNames global config file item
 *
 * @param driverName
 * @returns {Q.Promise} GlobalDriversConfigItem
 */
DriversState.prototype.readGlobalDriversConfigItemByName = function (driverName) {
    return readGlobalDriversConfig()
        .then(function (conf) {
            var driversConf = conf.drivers;

            return new GlobalDriversConfigItem(driversConf[driverName], driverName);
        });
};


/**
 * Writes driverNames global config file item
 *
 * @param globalDriversConfigItem {GlobalDriversConfigItem}
 * @returns {Q.Promise}
 */
DriversState.prototype.writeGlobalDriversConfigItem = function (globalDriversConfigItem) {
    return readGlobalDriversConfig()
        .then(function (conf) {
            conf.drivers[globalDriversConfigItem.driverName] = globalDriversConfigItem.toObject();

            return fspromised.writeFile(this._driversConfigFilepath,
                JsonConfigsUtils.prettyJsonStringify(conf));
        }.bind(this));
};


/**
 * Deletes driverNames global config file item
 *
 * @param driverName
 * @returns {Q.Promise}
 */
DriversState.prototype.deleteGlobalDriversConfigItemByName = function (driverName) {
    return readGlobalDriversConfig()
        .then(function (conf) {
            delete conf.drivers[driverName];
...
/**
 * Returns wrapper around driver's sourcecode
 *
 * @param driverName
 * @return {DriverModule}
 */
DriversState.prototype.getDriverModuleByName = function (driverName) {
    return new DriverModule(driverName, driversState.getDriverSourcecodePathByName(driverName));
};
