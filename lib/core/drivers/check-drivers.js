...
module.exports = function () {
    ...
    return driversState.readGlobalDriversConfig()
        .then(function (driversConf) {
            ...
            return Q.all(mountingDriversPromisesList(driversKeys, driversConf));
        })
        .then(function () {
...
function mountingDriversPromisesList(driversKeys, driversConf) {
    ...
    for (var i = 0; i < driversKeys.length; i++) {
        ...
        if (driverConf.status === "unmounted") {  // todo status ?!?!?!
        ...
        mountingDriversPromises.push(
            mountingDriverPromise(driverName, driverConf, mountingDriversPromises.length)
        );
    }
...
function mountingDriverPromise(driverName, driverConf, mountingDriversPromisesLength) {
