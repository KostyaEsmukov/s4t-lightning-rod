...
function mountingDriversPromisesList(globalDriversConfigItemList) {
    ...
    for (var i = 0; i < globalDriversConfigItemList.length; i++) {
        ...
        globalDriversConfigItem.autostart = JsonConfigsUtils.fixBoolFromString(globalDriversConfigItem.autostart);
        ...
        logger.info('[DRIVER] |--> ' + driverName + ' - status: ' + globalDriversConfigItem.status
            ...
            + ' - remote: { ' + globalDriversConfigItem.remote + ' , ' + globalDriversConfigItem.mirror_board + ' }');

        // todo Previously this cond included the 'isReconnected === true' - why?!
        if (globalDriversConfigItem.status === globalDriversConfigItem.STATUSES.UNMOUNTED) {  // todo status ?!?!?!
            // logger.debug("[DRIVER] - " + driverName
            //     + " --> It is not necessary restart this driver after reconnection!");
            continue;
        }

        if (globalDriversConfigItem.autostart !== true || globalDriversConfigItem.remote === null) {
        ...
        mountingDriversPromises.push(
            mountingDriverPromise(driverName, globalDriversConfigItem, mountingDriversPromises.length)
        );
    }
...
function mountingDriverPromise(driverName, globalDriversConfigItem, mountingDriversPromisesLength) {
    return Q.delay(2000 * mountingDriversPromisesLength)  // todo delay is hardcoded
        .then(function () {
            if (globalDriversConfigItem.status !== globalDriversConfigItem.STATUSES.MOUNTED) {  // todo status ?!?!?!
                return;
            }
